...
:
    ...
    (
        IOobject
        (
            IOobject::groupName("NormalStress", phase.name()),
            U.time().timeName(),
            U.mesh(),
            IOobject::NO_READ,
            IOobject::AUTO_WRITE
    ...
    (
        IOobject
        (
            IOobject::groupName("P_XX", phase.name()),
            U.time().timeName(),
            U.mesh(),
            IOobject::NO_READ,
            IOobject::AUTO_WRITE
    ...
    (
        IOobject
        (
            IOobject::groupName("P_YY", phase.name()),
            U.time().timeName(),
            U.mesh(),
            IOobject::NO_READ,
            IOobject::AUTO_WRITE
    ...
    (
        IOobject
        (
            IOobject::groupName("P_ZZ", phase.name()),
            U.time().timeName(),
            U.mesh(),
            IOobject::NO_READ,
            IOobject::AUTO_WRITE
...
void Foam::RASModels::kineticTheoryModel::correct()
{
    ...
    Info << "    max(Theta) = " << max(Theta_).value() << endl;
    {
        ...
        *frictionalStressModel_->frictionalPressure
        ...
        // total stress tensor
        dimensionedScalar rhodim_("rhodim_", dimensionSet(1,-3,0,0,0,0,0), 1.0); //just to make the dimensions correct in totStressTensor_
        volSymmTensorField totStressTensor_ (NormalStress_*symmTensor::I - rho*(nut_ + nuFric_)*dev(twoSymm(fvc::grad(U_)))- rho*(lambda_*fvc::div(phi_))*symmTensor::I);
          
        // diagonal components of the total stress tensor
        P_XX_ = totStressTensor_.component(symmTensor::XX);

        P_YY_ = totStressTensor_.component(symmTensor::YY);

        P_ZZ_ = totStressTensor_.component(symmTensor::ZZ);
