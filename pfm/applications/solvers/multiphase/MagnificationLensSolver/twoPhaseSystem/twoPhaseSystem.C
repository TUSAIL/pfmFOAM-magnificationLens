...
:
    ...
    (
         ...
         mesh_,
         dimensionedScalar("switchMagLens", dimless, 0.0)
    ),
    ...
    (
         ...
         mesh_,
         dimensionedScalar("differenceSource", dimless, 0.0)
    )    

{
    ...
    // meso-scale interphase force model
    interPhaseForce_.set
    (
        new BlendedInterfacialModel<interPhaseForceModel>
        (
            lookup("interPhaseForce"),
            (
                blendingMethods_.found("interPhaseForce")
              ? blendingMethods_["interPhaseForce"]
              : blendingMethods_["default"]
            ),
            pair_,
            pair1In2_,
...
// meso-scale interphase force model
Foam::tmp<Foam::volScalarField> Foam::twoPhaseSystem::CpIPh() const
{
...
//========================== Mass and Momentum coupling Magnification lens ===========================//
	IOdictionary magnificationLensDict
	(
	    IOobject
	    (
            "magnificationLensDict",
            "constant",
            mesh_,
            IOobject::MUST_READ,
            IOobject::NO_WRITE
    ...
    if (twoWayMagLensMMC) {
        // start coupling the mass based on discrete particles
        const volScalarField& voidfraction_ = mesh_.lookupObject<volScalarField>("voidfraction");
        ...
        const labelList& magLensCells = magLensCellSet.toc();
        forAll(magLensCells, i) {
    	    switchMagLens[magLensCells[i]] = 1.0;
        }
        if (mesh_.time().value() > mesh_.time().startTime().value() + magLensStart * mesh_.time().deltaTValue()) {
            forAll(dgdt_, celli) {
                if (dgdt_[celli] > 0.0) {
                   Sp[celli] -= (dgdt_[celli]/max(1 - alpha1[celli], 1e-4) + switchMagLens[celli]*(1.0/mesh_.time().deltaTValue()));
                   Su[celli] += (dgdt_[celli]/max(1 - alpha1[celli], 1e-4) + switchMagLens[celli]*(((1.0-voidfraction_[celli]))/mesh_.time().deltaTValue()));
                }
                else if (dgdt_[celli] < 0.0) {
                   Sp[celli] += (dgdt_[celli]/max(alpha1[celli], 1e-4) - switchMagLens[celli]*(1.0/mesh_.time().deltaTValue()));
                   Su[celli] += switchMagLens[celli]*(((1.0-voidfraction_[celli]))/mesh_.time().deltaTValue());
                }
            }
        } else {
            forAll(dgdt_, celli)
            {
                if (dgdt_[celli] > 0.0) {
                    Sp[celli] -= dgdt_[celli]/max(1 - alpha1[celli], 1e-4);
                    Su[celli] += dgdt_[celli]/max(1 - alpha1[celli], 1e-4);
                }
                else if (dgdt_[celli] < 0.0) {
